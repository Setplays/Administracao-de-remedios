...
# --- Configuração de Caminhos (DIAGNÓSTICO) ---
def resource_path(relative_path):
    """Retorna o caminho absoluto e verifica se existe"""
    try:
...
try:
    ...
    messagebox.showerror("Erro Fatal", f"Plyer não instalado: {e}")
...
try:
    ...
    messagebox.showerror("Erro Fatal", f"Pystray/Pillow erro: {e}")
...
# Validadores
def is_val_too_big(valores):
    for val in valores:
        if val > 10000000: return True
    return False

def is_str_too_big(word):
    if len(word) > 50: return True
    return False
...
class App:
    def __init__(self, root):
        ...
        # --- TESTE CRÍTICO DE ARQUIVOS ---
        # Isso vai nos dizer se o PyInstaller empacotou as imagens
        self.icon_png = resource_path("cardiogram.png")
        self.icon_ico = resource_path("cardiogram.ico")

        if not os.path.exists(self.icon_png):
            messagebox.showerror("ERRO DE ARQUIVO", f"Não encontrei o PNG em:\n{self.icon_png}")
        
        if not os.path.exists(self.icon_ico):
            # Não trava, mas avisa
            print(f"Aviso: ICO não encontrado em {self.icon_ico}")
        ...
        if TRAY_AVAILABLE and os.path.exists(self.icon_png):
    ...
    def _init_tray_icon(self):
        try:
            ...
            messagebox.showerror("Erro Tray", f"Falha ao criar bandeja: {e}")
        ...
        try:
            ...
            for rem_nome, estoque, unid, dose, dias_str, pac_nome in remedios:
                ...
                if dias_restantes <= 5:
                    ...
                    time.sleep(3) 
        except Exception as e: print(f"Erro Thread Notificação: {e}")
    ...
    def agendar_notificacao_main_thread(self, titulo, mensagem):
        try:
            ...
            notification.notify(title=titulo, message=mensagem, app_name="PharmaStock", app_icon=icon_path, timeout=5)
        except Exception as e: messagebox.showerror("Erro Notificação", f"Falha ao notificar: {e}")
    ...
    def testar_notificacao_agora(self):
        if not NOTIFIER_AVAILABLE:
            messagebox.showerror("Erro", "Plyer não está disponível.")
            return
        messagebox.showinfo("Teste", "Verificação iniciada... aguarde.")
...
if __name__ == "__main__":
    root = tk.Tk()
    try:
        icone_path = resource_path("cardiogram.png")
        if os.path.exists(icone_path):
            icone = ImageTk.PhotoImage(Image.open(icone_path))
            root.iconphoto(True, icone)
        else:
            # Isso vai aparecer se o ícone não foi empacotado
            messagebox.showerror("ERRO", f"Ícone da janela não encontrado:\n{icone_path}")
